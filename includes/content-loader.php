 ...
 */
function get_content_value($section, $key, $fallback = '') {
    global $site_content;
    ...
    foreach ($keys as $k) {
        if (is_array($content) && array_key_exists($k, $content)) {
            $content = $content[$k];
        } else {
            return $fallback;
        }
...
function display_text($section, $key, $fallback = '') {
    $value = get_content_value($section, $key, $fallback);
...
if (!function_exists('kaizen_link')) {
    function kaizen_link($url, $base = 'index.php') {
        ...
        if ($trimmed[0] === '#') {
            // Provide a clean #anchor string if we are currently on the index.php page.
            $current_script = basename($_SERVER['SCRIPT_NAME'] ?? '');
            if ($current_script === 'index.php' || $current_script === '') {
                return $trimmed;
            }
            return $base . $trimmed;
        }
