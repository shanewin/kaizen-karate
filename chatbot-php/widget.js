    ...
    async sendToChatbot(message) {
        try {
            ...
            const response = await fetch(this.config.apiEndpoint, {
                ...
                body: `question=${encodeURIComponent(message)}&conversation_history=${encodeURIComponent(JSON.stringify(conversationHistory))}`
            });
    ...
    formatMessageContent(content) {
        ...
        try {
            ...
            formatted = formatted.replace(urlRegex, (match) => {
                ...
                const trailingMatch = url.match(/(#[a-z0-9\-_]+)([A-Z][^\s<>")\]]*)$/);
                if (trailingMatch) {
                    url = url.replace(trailingMatch[0], trailingMatch[1]);
                    trailingText = trailingMatch[2];
                }

                const link = `<a href="${url}" target="_blank" rel="noopener noreferrer">${url}</a>`;
                return trailingText ? `${link} ${trailingText}` : link;
            });
