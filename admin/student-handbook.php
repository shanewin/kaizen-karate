...
$defaultHandbook = [
    ...
    'meta_description' => 'Explore the Kaizen Karate student handbook for dojo etiquette, belt requirements, and training expectations.',
    'content' => <<<'HTML'
      ...
      <div class="col-lg-10">
        ...
        <div class="policy-section mb-5">
          ...
          <div class="mb-4">
            ...
            <p class="mb-0">Weekend classes run in 2-month cycles. Other programs vary depending on the location. For details email us at <a href="mailto:coach.v@kaizenkaratemd.com">coach.v@kaizenkaratemd.com</a> or visit our list of classes <a href="index.php#weekend-evening">here</a>.</p>
          </div>
...
$handbookData = array_replace_recursive($defaultHandbook, $content['student_handbook_page'] ?? []);
$handbookKeyExists = array_key_exists('student_handbook_page', $content);

$legacyHandbookPlaceholder = 'Use this editor to build the full handbook content';
if (
    trim($handbookData['content'] ?? '') === '' ||
    strpos($handbookData['content'], $legacyHandbookPlaceholder) !== false ||
    strpos($handbookData['content'], '<h2>Student Handbook') !== false
) {
    $handbookData['content'] = $defaultHandbook['content'];
}

if (!$handbookKeyExists && $_SERVER['REQUEST_METHOD'] !== 'POST') {
    $content['student_handbook_page'] = $handbookData;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        $postedHandbook = $handbookData;

        $postedHandbook['enabled'] = isset($_POST['page_enabled']);
        $postedHandbook['page_title'] = sanitize_input($_POST['page_title'] ?? $postedHandbook['page_title']);
        $postedHandbook['meta_description'] = sanitize_input($_POST['meta_description'] ?? $postedHandbook['meta_description']);
        $postedHandbook['content'] = $_POST['page_content'] ?? $postedHandbook['content'];

        $content['student_handbook_page'] = $postedHandbook;
        if (save_json_data('site-content', $content)) {
            $message = success_message('Student handbook page saved to draft successfully!');
            $handbookData = $postedHandbook;
        } else {
            $message = error_message('Failed to save student handbook page.');
        }
    }
}
ob_start();
...
<div class="content-section">
    <h3 class="section-title"><i class="fas fa-user-graduate me-2"></i>Edit Handbook Content</h3>

    <form method="POST">
                            <input type="hidden" name="csrf_token" value="<?php echo htmlspecialchars(generate_csrf_token()); ?>">

                            <div class="form-check form-switch mb-4">
                                <input class="form-check-input" type="checkbox" id="page_enabled" name="page_enabled" <?php echo $handbookData['enabled'] ? 'checked' : ''; ?>>
                                <label class="form-check-label" for="page_enabled">Enable Student Handbook Page</label>
                            </div>

                            <div class="mb-3">
                                <label class="form-label"><strong>Page Title</strong></label>
                                <input type="text" class="form-control" name="page_title" value="<?php echo htmlspecialchars($handbookData['page_title']); ?>" placeholder="Student Handbook">
                            </div>

                            <div class="mb-3">
                                <label class="form-label"><strong>Meta Description</strong></label>
                                <textarea class="form-control" name="meta_description" rows="3" placeholder="Short summary for search results."><?php echo htmlspecialchars($handbookData['meta_description']); ?></textarea>
                            </div>

                            <div class="mb-4">
                                <label class="form-label"><strong>Handbook Page Content</strong></label>
                                <textarea class="form-control tinymce-editor" name="page_content" rows="15"><?php echo $handbookData['content']; ?></textarea>
                                <div class="form-text">Include sections, tables, or embeds as needed. You can link to downloadable PDFs from this content.</div>
                            </div>
                            ...
                            <div class="d-flex justify-content-between align-items-center">
                                ...
                                <button type="submit" class="btn btn-kaizen">
                                    <i class="fas fa-save me-2"></i>Save Handbook to Draft
                                </button>
                            </div>
                        </form>
...
$page_title = 'Student Handbook Page';
