...
if ($_POST && verify_csrf_token($_POST['csrf_token'] ?? '')) {
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'accordion_process') {
        ...
        $accordionData['summary'] = [
            'title' => $_POST['process_summary_title'] ?? 'In Summary',
            'icon' => $_POST['process_summary_icon'] ?? 'fas fa-info-circle',
            'text' => $_POST['process_summary_text'] ?? ''
        ];
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'accordion_requirements') {
        $accordionData = [
            ...
            'important_notice' => [
                ...
                'icon' => $_POST['notice_icon'] ?? 'fas fa-exclamation-circle',
                'main_text' => $_POST['notice_main_text'] ?? '',
                'special_notes' => $_POST['notice_special_notes'] ?? ''
            ],
            'requirement_images' => []
        ];
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'accordion_clothing') {
        $accordionData = [
            ...
            'content_type' => 'clothing',
            'section_title' => $_POST['clothing_section_title'] ?? 'Belt-Specific Clothing Requirements',
            'clothing_cards' => [],
            'shop_section' => [
        ...
        if (isset($_POST['clothing_card_id']) && is_array($_POST['clothing_card_id'])) {
            foreach ($_POST['clothing_card_id'] as $index => $cardId) {
                $clothingCard = [
                    ...
                    'requirement_text' => $_POST['clothing_requirement_text'][$index] ?? '',
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'accordion_scripts') {
        // Color mapping for belt colors (admin uses names, frontend needs hex)
        $colorMap = [
            'green' => '#28a745',
            'purple' => '#6f42c1',
            'blue' => '#007bff',
        ...
        $accordionData = [
            ...
            'important_note' => [
                ...
                // Combine description and additional_text into content field
                'content' => trim(
                    ($_POST['note_description'] ?? '') . 
                    (!empty($_POST['note_additional_text']) ? '<br>' . $_POST['note_additional_text'] : '')
                )
            ]
        ];
        ...
        if (isset($_POST['script_card_id']) && is_array($_POST['script_card_id'])) {
            foreach ($_POST['script_card_id'] as $index => $cardId) {
                $beltColor = $_POST['script_belt_color'][$index] ?? '';
                $stripeColor = $_POST['script_stripe_color'][$index] ?? '';
                ...
                // Add belt_color with hex value if set
                if (!empty($beltColor)) {
                    $scriptCard['belt_color'] = $colorMap[$beltColor] ?? $beltColor;
                }
                
                // Add has_stripe boolean if stripe color is set
                if (!empty($stripeColor)) {
                    $scriptCard['has_stripe'] = true;
                }
        ...
        $accordionData['lightbox_content'] = [
            'testing_tips' => $_POST['testing_tips_content'] ?? '',
            'video_instructions' => $_POST['video_instructions_content'] ?? '',
            'green_belt' => $_POST['green_belt_content'] ?? '',
            'purple_belt' => $_POST['purple_belt_content'] ?? '',
            'blue_belt' => $_POST['blue_belt_content'] ?? '',
            'brown_belt' => $_POST['brown_belt_content'] ?? '',
            'brown_stripe' => $_POST['brown_stripe_content'] ?? '',
            'red_belt' => $_POST['red_belt_content'] ?? '',
            'red_stripe' => $_POST['red_stripe_content'] ?? ''
        ];
        ...
        if (isset($_POST['date_card_id']) && is_array($_POST['date_card_id'])) {
            foreach ($_POST['date_card_id'] as $index => $cardId) {
                $dateCard = [
                    ...
                    'show_link' => isset($_POST['date_card_show_link'][$index]) && $_POST['date_card_show_link'][$index] == '1',
                    'invitation_only' => isset($_POST['date_card_invitation_only'][$index]) && $_POST['date_card_invitation_only'][$index] == '1',
            ...
            // Sort date cards by order to ensure correct sequence
            if (!empty($accordionData['date_cards'])) {
                usort($accordionData['date_cards'], function($a, $b) {
                    return ($a['order'] ?? 0) - ($b['order'] ?? 0);
                });
            }
        }
    ...
    if (isset($_POST['action']) && $_POST['action'] === 'accordion_registration') {
        $accordionData = [
            ...
            'order' => 6,
            'description' => $_POST['registration_accordion_description'] ?? 'Ready to register for your belt examination? Follow these steps:',
            'content_type' => 'registration',
            // Group Testing Section
            'group_testing_section' => [
                'title' => $_POST['group_testing_title'] ?? 'In-Person Group Testing',
                'icon' => $_POST['group_testing_icon'] ?? 'fas fa-users',
                'description' => $_POST['group_testing_description'] ?? 'The belt exam will be held IN-PERSON for all students testing for white with black stripe - red belt ranks. Students will take part in a group test / workshop hosted by Coach V. All material for the student\'s belt level will be covered in detail during this special event.'
            ],
            // Register Section Headers
            'register_section' => [
                'heading' => $_POST['register_heading'] ?? 'Register',
                'subtitle' => $_POST['register_subtitle'] ?? 'Click on your preferred testing date below'
            ],
            // Location Section Headers
            'location_section' => [
                'heading' => $_POST['location_heading'] ?? 'Testing Location',
                'icon' => $_POST['location_icon'] ?? 'fas fa-map-marker-alt'
            ],
            // Shared location data
            'shared_location' => [
                'name' => $_POST['registration_location_name'] ?? '',
                'address' => $_POST['registration_location_address'] ?? ''
            ],
            'registration_cards' => []
        ];
        
        // Process registration cards
        if (isset($_POST['registration_card_title']) && is_array($_POST['registration_card_title'])) {
            foreach ($_POST['registration_card_title'] as $index => $title) {
                $registrationCard = [
                    'title' => $title,
                    'registration_link' => $_POST['registration_card_link'][$index] ?? '',
                    'belt_colors' => $_POST['registration_card_belt_colors'][$index] ?? [],
                    'exam_type' => $_POST['registration_card_exam_type'][$index] ?? 'Youth Belt Exam',
                    'belt_levels' => $_POST['registration_card_belt_levels'][$index] ?? '',
                    'button_text' => $_POST['registration_card_button'][$index] ?? 'Register Now',
                    'invitation_only' => isset($_POST['registration_card_invitation_only'][$index]) && $_POST['registration_card_invitation_only'][$index] == '1',
        ...
        // Process pre-registration requirements
        $requirementsText = $_POST['requirements_items'] ?? '';
        $requirementsArray = array_filter(array_map('trim', explode("\n", $requirementsText)));
        
        if (empty($requirementsArray)) {
            $requirementsArray = [
                'You must receive written approval from the Kaizen office team',
                'Registration closes one week before the testing date',
                'Testing fees must be paid at time of registration'
            ];
        }
        
        $accordionData['pre_registration_requirements'] = [
            'title' => $_POST['requirements_title'] ?? 'Before You Register',
            'icon' => $_POST['requirements_icon'] ?? 'fas fa-exclamation-triangle',
...
<body>
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-9 col-lg-10 p-4">
                ...
                <div class="content-section">
                    <h3 class="section-title" data-bs-toggle="collapse" data-bs-target="#dates-collapse" style="cursor: pointer;">
                        <i class="fas fa-calendar-alt me-2"></i>Upcoming Testing Dates
                    ...
                    <form method="POST">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select class="form-control icon-select" name="date_card_icon[]">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <input type="number" class="form-control" name="date_card_order[]" 
                                                   value="<?php echo htmlspecialchars($card['order'] ?? 1); ?>" min="1">
                                        </div>
                                    </div>
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" class="form-control" name="date_card_location_name[]"
                                                   ...
                                                   placeholder="e.g., East Silver Spring ES - GYM">
                                        </div>
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" class="form-control" name="date_card_street_address[]"
                                                   ...
                                                   placeholder="e.g., 631 Silver Spring Ave">
                                        </div>
                                    </div>
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" class="form-control" name="date_card_city_state_zip[]"
                                                   ...
                                                   placeholder="e.g., Silver Spring, MD 20910">
                                        </div>
                                        <div class="col-md-6">
                                            ...
                                            <input type="text" class="form-control" name="date_card_datetime_string[]"
                                                   ...
                                                   placeholder="e.g., Saturday, November 15th - 11:00 AM Start Time">
                                        </div>
                                    </div>
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-8">
                                            ...
                                            <div class="form-check mt-2">
                                                <input type="hidden" name="date_card_show_link[<?php echo $index; ?>]" value="0">
                                                <input type="checkbox" class="form-check-input" name="date_card_show_link[<?php echo $index; ?>]" 
                                                       id="show_link_<?php echo $index; ?>" value="1"
                                                       <?php echo ($card['show_link'] ?? true) ? 'checked' : ''; ?>>
                                                <label class="form-check-label" for="show_link_<?php echo $index; ?>">
                                                    Show registration link on public site
                                                </label>
                                            </div>
                                        </div>
                                        <div class="col-md-4">
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-12">
                                            <div class="form-check">
                                                <input type="hidden" name="date_card_invitation_only[<?php echo $index; ?>]" value="0">
                                                <input type="checkbox" class="form-check-input" name="date_card_invitation_only[<?php echo $index; ?>]" 
                                                       id="invitation_only_<?php echo $index; ?>" value="1"
                                                       <?php echo ($card['invitation_only'] ?? false) ? 'checked' : ''; ?>>
                                                <label class="form-check-label" for="invitation_only_<?php echo $index; ?>">
                                                    <strong style="color: #dc3545;">Invitation Only</strong> - Display "Invitation Only" badge on this date
                                                </label>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="submit" class="btn btn-kaizen">
                                <i class="fas fa-save"></i> Save Testing Dates
                            </button>
                        </div>
                    </form>
                    </div>
                ...
                <div class="content-section">
                    <h3 class="section-title" data-bs-toggle="collapse" data-bs-target="#process-collapse" style="cursor: pointer;">
                        <i class="fas fa-list-check me-2"></i>Testing Process
                        <i class="fas fa-chevron-down float-end"></i>
                    </h3>
                    ...
                    <form method="POST">
                        ...
                        <div class="card mb-4" style="background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.1);">
                            <div class="card-header" style="background: rgba(220, 53, 69, 0.1); border-bottom: 1px solid rgba(255, 255, 255, 0.1);">
                                <h5 class="mb-0" style="color: #dc3545;">
                                    <i class="fas fa-info-circle me-2"></i>Summary Section
                                </h5>
                            </div>
                            <div class="card-body">
                                <div class="row mb-3">
                                    <div class="col-md-6">
                                        ...
                                        <input type="text" class="form-control" name="process_summary_title"
                                               value="<?php echo htmlspecialchars($processAccordion['summary']['title'] ?? 'In Summary'); ?>"
                                               placeholder="e.g., In Summary">
                                    </div>
                                    <div class="col-md-6">
                                        <label class="form-label">Summary Icon</label>
                                        <select class="form-control" name="process_summary_icon">
                                            <option value="fas fa-info-circle" <?php echo ($processAccordion['summary']['icon'] ?? 'fas fa-info-circle') === 'fas fa-info-circle' ? 'selected' : ''; ?>>ℹ️ Info Circle</option>
                                            <option value="fas fa-check-circle" <?php echo ($processAccordion['summary']['icon'] ?? '') === 'fas fa-check-circle' ? 'selected' : ''; ?>>✓ Check Circle</option>
                                            <option value="fas fa-exclamation-circle" <?php echo ($processAccordion['summary']['icon'] ?? '') === 'fas fa-exclamation-circle' ? 'selected' : ''; ?>>⚠️ Exclamation Circle</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="row mb-3">
                                    <div class="col-md-12">
                                        <label class="form-label">Summary Content</label>
                                        <textarea class="form-control" name="process_summary_text" rows="3"
                                                  placeholder="Enter summary text..."><?php echo htmlspecialchars($processAccordion['summary']['text'] ?? ''); ?></textarea>
                                        <small class="form-text text-muted">Use &lt;br&gt; for line breaks if needed</small>
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    </div> <!-- End process-collapse -->
                </div> <!-- End Testing Process content-section -->
                <!-- Requirements to Test Accordion -->
                <div class="content-section">
                    <h3 class="section-title" data-bs-toggle="collapse" data-bs-target="#requirements-collapse" style="cursor: pointer;">
                        <i class="fas fa-clipboard-list me-2"></i>Requirements to Test Accordion
                        <i class="fas fa-chevron-down float-end"></i>
                    </h3>
                    <div class="collapse" id="requirements-collapse">
                        ...
                        <div class="card mb-4">
                            <div class="card-body">
                                <div class="row mb-3">
                                    ...
                                    <div class="col-md-6">
                                        <label class="form-label">Notice Icon</label>
                                        <select class="form-control" name="notice_icon">
                                            <option value="fas fa-exclamation-circle" <?php echo ($requirementsAccordion['important_notice']['icon'] ?? 'fas fa-exclamation-circle') === 'fas fa-exclamation-circle' ? 'selected' : ''; ?>>⚠️ Exclamation Circle</option>
                                            <option value="fas fa-info-circle" <?php echo ($requirementsAccordion['important_notice']['icon'] ?? '') === 'fas fa-info-circle' ? 'selected' : ''; ?>>ℹ️ Info Circle</option>
                                            <option value="fas fa-bell" <?php echo ($requirementsAccordion['important_notice']['icon'] ?? '') === 'fas fa-bell' ? 'selected' : ''; ?>>🔔 Bell</option>
                                        </select>
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <textarea class="form-control" name="notice_main_text" rows="4" placeholder="Enter the main explanation text..."><?php echo htmlspecialchars($requirementsAccordion['important_notice']['main_text'] ?? "The requirements listed below are minimums.\nPlease keep in mind that the instructor of the class may require more than what is posted."); ?></textarea>
                                    <small class="form-text text-muted">Use line breaks to separate paragraphs</small>
                                </div>
                                
                                <div class="mb-3">
                                    <label class="form-label">Special Notes</label>
                                    <textarea class="form-control" name="notice_special_notes" rows="3" placeholder="Enter special notes or restrictions..."><?php echo htmlspecialchars($requirementsAccordion['important_notice']['special_notes'] ?? "*Students who train at Summer Camp only can test up to green belt rank.\n*In order to test for purple belt and higher, student MUST attend weekend and / or evening classes year-round."); ?></textarea>
                                    <small class="form-text text-muted">Use asterisks (*) to create bullet points</small>
                                </div>
                            </div>
                        </div>
                    ...
                    </div> <!-- End requirements-collapse -->
                </div> <!-- End Requirements to Test content-section -->
                
                <!-- Belt Exam Clothing Requirements Section -->
                <div class="content-section">
                    <h3 class="section-title" data-bs-toggle="collapse" data-bs-target="#clothing-collapse" style="cursor: pointer;">
                        <i class="fas fa-tshirt me-2"></i>Belt Exam Clothing Requirements
                        <i class="fas fa-chevron-down float-end"></i>
                    </h3>
                    <div class="collapse" id="clothing-collapse">
                    ...
                    <form method="POST">
                        ...
                        <!-- Section Title for Belt-Specific Requirements -->
                        <div class="row mb-3">
                            <div class="col-md-12">
                                <label class="form-label">Section Title</label>
                                <input type="text" class="form-control" name="clothing_section_title" 
                                       value="<?php echo htmlspecialchars($clothingAccordion['section_title'] ?? 'Belt-Specific Clothing Requirements'); ?>" 
                                       placeholder="Belt-Specific Clothing Requirements">
                                <small class="form-text text-muted">This appears as the main heading above the belt cards</small>
                            </div>
                        </div>
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-12">
                                            ...
                                            <textarea class="form-control" name="clothing_requirement_text[]" rows="2" 
                                                      placeholder="e.g., Need a black Kaizen Karate t-shirt prior to testing for this belt"
                                                      required><?php echo htmlspecialchars($card['requirement_text'] ?? ''); ?></textarea>
                                            <small class="form-text text-muted">Describe what's needed for this belt level.</small>
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="row mb-3">
                                    ...
                                    <div class="col-md-6">
                                        ...
                                        <input type="url" class="form-control" name="shop_url"
                                               value="<?php echo htmlspecialchars($clothingAccordion['shop_section']['shop_url'] ?? 'https://kaizenkarate.store/'); ?>" 
                                               placeholder="https://kaizenkarate.store/" required>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="submit" class="btn btn-kaizen">
                                <i class="fas fa-save"></i> Save Clothing Requirements
                            </button>
                        </div>
                    </form>
                    </div>
                ...
                <div class="content-section">
                    <h3 class="section-title" data-bs-toggle="collapse" data-bs-target="#scripts-collapse" style="cursor: pointer;">
                        <i class="fas fa-scroll me-2"></i>Testing Scripts
                        <i class="fas fa-chevron-down float-end"></i>
                    </h3>
                    ...
                    // Reverse color mapping for displaying hex colors as names in form
                    $reverseColorMap = [
                        '#28a745' => 'green',
                        '#6f42c1' => 'purple',
                        '#007bff' => 'blue',
                    ...
                    <form method="POST">
                                ...
                                <div class="card-body">
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-md-3">
                                            <label class="form-label">Belt Color</label>
                                            <?php 
                                            // Convert hex color back to name for form display
                                            $cardBeltColor = $card['belt_color'] ?? '';
                                            $selectedColor = $reverseColorMap[$cardBeltColor] ?? $cardBeltColor;
                                            ?>
                                            <select class="form-control" name="script_belt_color[]">
                                                ...
                                                <option value="purple" <?php echo $selectedColor === 'purple' ? 'selected' : ''; ?>>Purple</option>
                                                <option value="blue" <?php echo $selectedColor === 'blue' ? 'selected' : ''; ?>>Blue</option>
                                                <option value="brown" <?php echo $selectedColor === 'brown' ? 'selected' : ''; ?>>Brown</option>
                                                <option value="red" <?php echo $selectedColor === 'red' ? 'selected' : ''; ?>>Red</option>
                                                <option value="black" <?php echo $selectedColor === 'black' ? 'selected' : ''; ?>>Black</option>
                                            </select>
                                        </div>
                                        <div class="col-md-3">
                                            <label class="form-label">Order</label>
                                            <input type="number" class="form-control" name="script_card_order[]" 
                                                   value="<?php echo $card['order']; ?>" min="1" max="99" required>
                                        </div>
                                    </div>
                                    
                                    <div class="row mb-3">
                                        <div class="col-md-4">
                                            <label class="form-label">Has Stripe?</label>
                                            <?php 
                                            // Check if card has stripe (from has_stripe field or stripe_color field)
                                            $hasStripe = !empty($card['has_stripe']) || !empty($card['stripe_color']);
                                            ?>
                                            <select class="form-control" name="script_stripe_color[]">
                                                <option value="">No Stripe</option>
                                                <option value="stripe" <?php echo $hasStripe ? 'selected' : ''; ?>>Has Stripe</option>
                                            </select>
                                            <small class="form-text text-muted">Select if this is a stripe belt (e.g., Brown + Stripe)</small>
                                        </div>
                                        <div class="col-md-8">
                                            <label class="form-label">Lightbox Function</label>
                        ...
                        <div class="card mb-4">
                            <div class="card-body">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <label class="form-label">Note Content (HTML supported)</label>
                                        <?php 
                                        // Get existing content or build from legacy fields
                                        $noteContent = $scriptsAccordion['important_note']['content'] ?? '';
                                        if (empty($noteContent) && !empty($scriptsAccordion['important_note']['description'])) {
                                            $noteContent = $scriptsAccordion['important_note']['description'];
                                            if (!empty($scriptsAccordion['important_note']['additional_text'])) {
                                                $noteContent .= '<br>' . $scriptsAccordion['important_note']['additional_text'];
                                            }
                                        }
                                        ?>
                                        <textarea class="form-control" name="note_description" rows="3" required><?php echo htmlspecialchars($noteContent ?: 'Click any card above to view detailed information. Testing scripts are provided as study guides.<br>Actual testing may vary based on instructor discretion and individual student needs.'); ?></textarea>
                                        <small class="form-text text-muted">You can use HTML tags like &lt;br&gt; for line breaks. This will be displayed below the script cards.</small>
                                    </div>
                                </div>
                                <!-- Hidden field for backwards compatibility -->
                                <input type="hidden" name="note_additional_text" value="">
                            </div>
                        </div>
                        
                        <!-- Lightbox Content Management Card -->
                        <div class="card mt-4">
                            <div class="card-header bg-info text-white">
                                <h5 class="mb-0">
                                    <i class="fas fa-window-restore"></i> Lightbox Content Management
                                </h5>
                                <small>Edit the content that appears in each Testing Script lightbox popup</small>
                            </div>
                            <div class="card-body">
                                <!-- 1. Testing Tips -->
                                <div class="mb-5 pb-3 border-bottom">
                                    <label class="form-label fw-bold">
                                        ...
                                        Testing Tips Lightbox Content
                                    </label>
                                    <textarea class="tinymce-editor" name="testing_tips_content" rows="10"><?php echo htmlspecialchars($scriptsAccordion['lightbox_content']['testing_tips'] ?? ''); ?></textarea>
                                </div>
                                ...
                                <div class="mb-5 pb-3 border-bottom">
                                    <label class="form-label fw-bold">
                                        ...
                                        Video Instructions Lightbox Content
                                    </label>
                                    <textarea class="tinymce-editor" name="video_instructions_content" rows="10"><?php echo htmlspecialchars($scriptsAccordion['lightbox_content']['video_instructions'] ?? ''); ?></textarea>
                                </div>
                                ...
                                <div class="mb-5 pb-3 border-bottom">
                                    <label class="form-label fw-bold">
                                        ...
                                        Green Belt Script Content
                                    </label>
                                    <textarea class="tinymce-editor" name="green_belt_content" rows="10"><?php echo htmlspecialchars($scriptsAccordion['lightbox_content']['green_belt'] ?? ''); ?></textarea>
                                </div>

                                <!-- 4. Purple Belt -->
                                <div class="mb-5 pb-3 border-bottom">
                                    <label class="form-label fw-bold">
                                        <span class="badge" style="background-color: #6f42c1;">4</span>
                                        Purple Belt Script Content
                                    </label>
                                    <textarea class="tinymce-editor" name="purple_belt_content" rows="10"><?php echo htmlspecialchars($scriptsAccordion['lightbox_content']['purple_belt'] ?? ''); ?></textarea>
                                </div>
                                ...
                                <div class="mb-5 pb-3 border-bottom">
                                    <label class="form-label fw-bold">
                                        ...
                                        Blue Belt Script Content
                                    </label>
                                    <textarea class="tinymce-editor" name="blue_belt_content" rows="10"><?php echo htmlspecialchars($scriptsAccordion['lightbox_content']['blue_belt'] ?? ''); ?></textarea>
                                </div>

                                <!-- 6. Brown Belt -->
                                <div class="mb-5 pb-3 border-bottom">
                                    <label class="form-label fw-bold">
                                        <span class="badge" style="background-color: #8b4513;">6</span>
                                        Brown Belt Script Content
                                    </label>
                                    <textarea class="tinymce-editor" name="brown_belt_content" rows="10"><?php echo htmlspecialchars($scriptsAccordion['lightbox_content']['brown_belt'] ?? ''); ?></textarea>
                                </div>

                                <!-- 7. Brown Belt w/ Black Stripe -->
                                <div class="mb-5 pb-3 border-bottom">
                                    <label class="form-label fw-bold">
                                        <span class="badge" style="background-color: #8b4513; position: relative;">
                                            7
                                            <span style="position: absolute; top: 50%; left: 0; right: 0; height: 2px; background: #000;"></span>
                                        </span>
                                        Brown Belt w/ Black Stripe Content
                                    </label>
                                    <textarea class="tinymce-editor" name="brown_stripe_content" rows="10"><?php echo htmlspecialchars($scriptsAccordion['lightbox_content']['brown_stripe'] ?? ''); ?></textarea>
                                </div>
                                ...
                                <div class="mb-5 pb-3 border-bottom">
                                    <label class="form-label fw-bold">
                                        ...
                                        Red Belt Script Content
                                    </label>
                                    <textarea class="tinymce-editor" name="red_belt_content" rows="10"><?php echo htmlspecialchars($scriptsAccordion['lightbox_content']['red_belt'] ?? ''); ?></textarea>
                                </div>

                                <!-- 9. Red Belt w/ Black Stripe -->
                                <div class="mb-5 pb-3">
                                    <label class="form-label fw-bold">
                                        <span class="badge bg-danger me-2" style="position: relative;">
                                            9
                                            <span style="position: absolute; top: 50%; left: 0; right: 0; height: 2px; background: #000;"></span>
                                        </span>
                                        Red Belt w/ Black Stripe Content
                                    </label>
                                    <textarea class="tinymce-editor" name="red_stripe_content" rows="10"><?php echo htmlspecialchars($scriptsAccordion['lightbox_content']['red_stripe'] ?? ''); ?></textarea>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="d-flex justify-content-end">
                            <button type="submit" class="btn btn-kaizen">
                                <i class="fas fa-save"></i> Save Testing Scripts
                            </button>
                        </div>
                    </form>
                    </div> <!-- End scripts-collapse -->
                </div> <!-- End Testing Scripts content-section -->
                
                <!-- Register for Belt Exam Section -->
                <div class="content-section">
                    <h3 class="section-title" data-bs-toggle="collapse" data-bs-target="#register-collapse" style="cursor: pointer;">
                        <i class="fas fa-user-plus me-2"></i>Register for Belt Exam
                        <i class="fas fa-chevron-down float-end"></i>
                    </h3>
                    ...
                    <form method="POST">
                        ...
                        <div class="card mb-4" style="background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.1);">
                            <div class="card-header" style="background: rgba(220, 53, 69, 0.1); border-bottom: 1px solid rgba(255, 255, 255, 0.1);">
                                <h5 class="mb-0" style="color: #dc3545;"><i class="fas fa-users me-2"></i>In-Person Group Testing Section</h5>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="group_testing_title" class="form-label">Section Title</label>
                                            <input type="text" class="form-control" id="group_testing_title" 
                                                   name="group_testing_title"
                                                   value="<?php echo htmlspecialchars($registrationAccordion['group_testing_section']['title'] ?? 'In-Person Group Testing'); ?>"
                                                   placeholder="e.g., In-Person Group Testing">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="group_testing_icon" class="form-label">Section Icon</label>
                                            <select class="form-control" id="group_testing_icon" name="group_testing_icon">
                                                <option value="fas fa-users" <?php echo ($registrationAccordion['group_testing_section']['icon'] ?? 'fas fa-users') === 'fas fa-users' ? 'selected' : ''; ?>>👥 Users</option>
                                                <option value="fas fa-people-arrows" <?php echo ($registrationAccordion['group_testing_section']['icon'] ?? '') === 'fas fa-people-arrows' ? 'selected' : ''; ?>>🔄 People Arrows</option>
                                                <option value="fas fa-user-friends" <?php echo ($registrationAccordion['group_testing_section']['icon'] ?? '') === 'fas fa-user-friends' ? 'selected' : ''; ?>>👫 User Friends</option>
                                                <option value="fas fa-chalkboard-teacher" <?php echo ($registrationAccordion['group_testing_section']['icon'] ?? '') === 'fas fa-chalkboard-teacher' ? 'selected' : ''; ?>>👨‍🏫 Teacher</option>
                                            </select>
                                        </div>
                                    </div>
                                    <div class="col-12">
                                        <div class="mb-3">
                                            <label for="group_testing_description" class="form-label">Description</label>
                                            <textarea class="form-control" id="group_testing_description" name="group_testing_description" 
                                                      rows="3" placeholder="Enter the group testing description"><?php echo htmlspecialchars($registrationAccordion['group_testing_section']['description'] ?? 'The belt exam will be held IN-PERSON for all students testing for white with black stripe - red belt ranks. Students will take part in a group test / workshop hosted by Coach V. All material for the student\'s belt level will be covered in detail during this special event.'); ?></textarea>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card mb-4" style="background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.1);">
                            <div class="card-header" style="background: rgba(220, 53, 69, 0.1); border-bottom: 1px solid rgba(255, 255, 255, 0.1);">
                                <h5 class="mb-0" style="color: #dc3545;"><i class="fas fa-heading me-2"></i>Register Section Headers</h5>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="register_heading" class="form-label">Main Heading</label>
                                            <input type="text" class="form-control" id="register_heading" 
                                                   name="register_heading"
                                                   value="<?php echo htmlspecialchars($registrationAccordion['register_section']['heading'] ?? 'Register'); ?>"
                                                   placeholder="e.g., Register">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="register_subtitle" class="form-label">Subtitle</label>
                                            <input type="text" class="form-control" id="register_subtitle" 
                                                   name="register_subtitle"
                                                   value="<?php echo htmlspecialchars($registrationAccordion['register_section']['subtitle'] ?? 'Click on your preferred testing date below'); ?>"
                                                   placeholder="e.g., Click on your preferred testing date below">
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card mb-4" style="background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.1);">
                            <div class="card-header" style="background: rgba(220, 53, 69, 0.1); border-bottom: 1px solid rgba(255, 255, 255, 0.1);">
                                <h5 class="mb-0" style="color: #dc3545;"><i class="fas fa-map-signs me-2"></i>Location Section Headers</h5>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="location_heading" class="form-label">Location Heading</label>
                                            <input type="text" class="form-control" id="location_heading" 
                                                   name="location_heading"
                                                   value="<?php echo htmlspecialchars($registrationAccordion['location_section']['heading'] ?? 'Testing Location'); ?>"
                                                   placeholder="e.g., Testing Location">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="location_icon" class="form-label">Location Icon</label>
                                            <select class="form-control" id="location_icon" name="location_icon">
                                                <option value="fas fa-map-marker-alt" <?php echo ($registrationAccordion['location_section']['icon'] ?? 'fas fa-map-marker-alt') === 'fas fa-map-marker-alt' ? 'selected' : ''; ?>>📍 Map Marker</option>
                                                <option value="fas fa-location-arrow" <?php echo ($registrationAccordion['location_section']['icon'] ?? '') === 'fas fa-location-arrow' ? 'selected' : ''; ?>>➤ Location Arrow</option>
                                                <option value="fas fa-map-pin" <?php echo ($registrationAccordion['location_section']['icon'] ?? '') === 'fas fa-map-pin' ? 'selected' : ''; ?>>📌 Map Pin</option>
                                                <option value="fas fa-building" <?php echo ($registrationAccordion['location_section']['icon'] ?? '') === 'fas fa-building' ? 'selected' : ''; ?>>🏢 Building</option>
                                            </select>
                                        </div>
                                    </div>
                                </div>
                        ...
                        <!-- Shared Testing Location -->
                        <div class="card mb-4" style="background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.1);">
                            <div class="card-header" style="background: rgba(220, 53, 69, 0.1); border-bottom: 1px solid rgba(255, 255, 255, 0.1);">
                                <h5 class="mb-0" style="color: #dc3545;"><i class="fas fa-map-marker-alt me-2"></i>Testing Location (Shared for All Cards)</h5>
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="registration_location_name" class="form-label">Location Name</label>
                                            <input type="text" class="form-control" id="registration_location_name" 
                                                   name="registration_location_name"
                                                   value="<?php echo htmlspecialchars($registrationAccordion['shared_location']['name'] ?? 'East Silver Spring Elementary School'); ?>"
                                                   placeholder="e.g., East Silver Spring Elementary School">
                                        </div>
                                    </div>
                                    <div class="col-md-6">
                                        <div class="mb-3">
                                            <label for="registration_location_address" class="form-label">Address</label>
                                            <input type="text" class="form-control" id="registration_location_address" 
                                                   name="registration_location_address"
                                                   value="<?php echo htmlspecialchars($registrationAccordion['shared_location']['address'] ?? '631 Silver Spring Ave, Silver Spring, MD 20910 - GYM'); ?>"
                                                   placeholder="e.g., 631 Silver Spring Ave, Silver Spring, MD 20910 - GYM">
                                        </div>
                                    </div>
                                </div>
                            </div>
                                ...
                                <div class="card-body">
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <select class="form-control" name="registration_card_exam_type[]" required>
                                                <option value="Youth Belt Exam" <?php echo ($card['exam_type'] ?? $card['description'] ?? '') === 'Youth Belt Exam' ? 'selected' : ''; ?>>Youth Belt Exam</option>
                                                <option value="Adult Belt Exam" <?php echo ($card['exam_type'] ?? $card['description'] ?? '') === 'Adult Belt Exam' ? 'selected' : ''; ?>>Adult Belt Exam</option>
                                                <option value="Makeup Youth Belt Exam" <?php echo ($card['exam_type'] ?? $card['description'] ?? '') === 'Makeup Youth Belt Exam' ? 'selected' : ''; ?>>Makeup Youth Belt Exam</option>
                                                <option value="Makeup Adult Belt Exam" <?php echo ($card['exam_type'] ?? $card['description'] ?? '') === 'Makeup Adult Belt Exam' ? 'selected' : ''; ?>>Makeup Adult Belt Exam</option>
                                            </select>
                                        </div>
                                        <div class="col-md-4">
                                            <label class="form-label">Order</label>
                                            <input type="number" class="form-control" name="registration_card_order[]" 
                                                   value="<?php echo $card['order']; ?>" min="1" max="10" required>
                                        </div>
                                    </div>
                                    
                                    <div class="row mb-3">
                                        <div class="col-md-6">
                                            <label class="form-label">Belt Levels (Text Description)</label>
                                            <input type="text" class="form-control" name="registration_card_belt_levels[]"
                                                   value="<?php echo htmlspecialchars($card['belt_levels']); ?>" 
                                                   placeholder="e.g., White belts with black stripe only" required>
                                        </div>
                                        <div class="col-md-6">
                                            <label class="form-label">Belt Colors (Visual Graphics)</label>
                                            <select class="form-control" name="registration_card_belt_colors[<?php echo $index; ?>][]" multiple size="5">
                                                <?php 
                                                $available_colors = [
                                                    'white_orange' => 'White w/Orange Stripe',
                                                    'white_black' => 'White w/Black Stripe',
                                                    'orange' => 'Orange',
                                                    'yellow' => 'Yellow',
                                                    'green' => 'Green',
                                                    'purple' => 'Purple',
                                                ...
                                                $selected_colors = $card['belt_colors'] ?? [];
                                                foreach ($available_colors as $value => $label): 
                                                ?>
                                                <option value="<?php echo $value; ?>" 
                                                        <?php echo in_array($value, $selected_colors) ? 'selected' : ''; ?>>
                                                    <?php echo $label; ?>
                                                </option>
                                                <?php endforeach; ?>
                                            </select>
                                    ...
                                    <div class="row mb-3">
                                        ...
                                        <div class="col-md-4">
                                            ...
                                            <input type="text" class="form-control" name="registration_card_button[]"
                                                   value="<?php echo htmlspecialchars($card['button_text'] ?? 'Register Now'); ?>"
                                                   placeholder="e.g., Register Now" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="row mb-3">
                                        <div class="col-md-12">
                                            <div class="form-check">
                                                <input type="hidden" name="registration_card_invitation_only[<?php echo $index; ?>]" value="0">
                                                <input type="checkbox" class="form-check-input" name="registration_card_invitation_only[<?php echo $index; ?>]" 
                                                       id="reg_invitation_only_<?php echo $index; ?>" value="1"
                                                       <?php echo ($card['invitation_only'] ?? false) ? 'checked' : ''; ?>>
                                                <label class="form-check-label" for="reg_invitation_only_<?php echo $index; ?>">
                                                    <strong style="color: #dc3545;">Invitation Only</strong> - Display "Invitation Only" badge on this registration card
                                                </label>
                                            </div>
                                        </div>
                                    </div>
                        ...
                        <div class="card mb-4" style="background: rgba(255, 255, 255, 0.05); border: 1px solid rgba(255, 255, 255, 0.1);">
                            <div class="card-header" style="background: rgba(220, 53, 69, 0.1); border-bottom: 1px solid rgba(255, 255, 255, 0.1);">
                                <h5 class="mb-0" style="color: #dc3545;"><i class="fas fa-exclamation-triangle me-2"></i>Pre-Registration Requirements</h5>
                            </div>
                            <div class="card-body">
                                <div class="row mb-3">
                                    <div class="col-md-6">
                                        <label for="requirements_title" class="form-label">Section Title</label>
                                        <input type="text" class="form-control" id="requirements_title" name="requirements_title"
                                               value="<?php echo htmlspecialchars($registrationAccordion['pre_registration_requirements']['title'] ?? 'Before You Register'); ?>"
                                               placeholder="e.g., Before You Register">
                                    </div>
                                    <div class="col-md-6">
                                        <label for="requirements_icon" class="form-label">Section Icon</label>
                                        <select class="form-control" id="requirements_icon" name="requirements_icon">
                                            <option value="fas fa-exclamation-triangle" <?php echo ($registrationAccordion['pre_registration_requirements']['icon'] ?? 'fas fa-exclamation-triangle') === 'fas fa-exclamation-triangle' ? 'selected' : ''; ?>>⚠️ Warning Triangle</option>
                                            <option value="fas fa-info-circle" <?php echo ($registrationAccordion['pre_registration_requirements']['icon'] ?? '') === 'fas fa-info-circle' ? 'selected' : ''; ?>>ℹ️ Info Circle</option>
                                            <option value="fas fa-clipboard-check" <?php echo ($registrationAccordion['pre_registration_requirements']['icon'] ?? '') === 'fas fa-clipboard-check' ? 'selected' : ''; ?>>✅ Clipboard Check</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="mb-3">
                                    <label class="form-label">Requirements (One per line)</label>
                                    <textarea class="form-control" name="requirements_items" rows="5" placeholder="Enter each requirement on a new line"><?php 
                                        if (!empty($registrationAccordion['pre_registration_requirements']['requirements'])) {
                                            echo htmlspecialchars(implode("\n", $registrationAccordion['pre_registration_requirements']['requirements']));
                                        } else {
                                            echo "You must receive written approval from the Kaizen office team\nRegistration closes one week before the testing date\nTesting fees must be paid at time of registration";
                                        }
                                    ?></textarea>
                    ...
                    </div> <!-- End register-collapse -->
                </div> <!-- End Register for Belt Exam content-section -->
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Removed old clothing requirement handlers - no longer needed with single textarea
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-3">
                                <label class="form-label">Order</label>
                                <input type="number" class="form-control" name="script_card_order[]" 
                                       value="${newIndex + 1}" min="1" max="99" required>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-4">
                                ...
                                <select class="form-control" name="script_stripe_color[]">
                                    <option value="">No Stripe</option>
                                    <option value="stripe">Has Stripe</option>
                                </select>
                                <small class="form-text text-muted">Select if this is a stripe belt (e.g., Brown + Stripe)</small>
                            </div>
                            <div class="col-md-8">
                ...
                newCard.innerHTML = `
                    ...
                    <div class="card-body">
                        ...
                        <div class="row mb-3">
                            <div class="col-md-8">
                                ...
                                <input type="text" class="form-control" name="date_card_link_text[]"
                                       ...
                                       placeholder="For more details and registration, click here">
                                <div class="form-check mt-2">
                                    <input type="hidden" name="date_card_show_link[NEW_INDEX]" value="0">
                                    <input type="checkbox" class="form-check-input" name="date_card_show_link[NEW_INDEX]" 
                                           id="show_link_NEW_INDEX" value="1" checked>
                                    <label class="form-check-label" for="show_link_NEW_INDEX">
                                        Show registration link on public site
                                    </label>
                                </div>
                            </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                <div class="form-check">
                                    <input type="hidden" name="date_card_invitation_only[NEW_INDEX]" value="0">
                                    <input type="checkbox" class="form-check-input" name="date_card_invitation_only[NEW_INDEX]" 
                                           id="invitation_only_NEW_INDEX" value="1">
                                    <label class="form-check-label" for="invitation_only_NEW_INDEX">
                                        <strong style="color: #dc3545;">Invitation Only</strong> - Display "Invitation Only" badge on this date
                                    </label>
                                </div>
                            </div>
                    ...
                    <div class="card-body">
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                <label class="form-label">Exam Type</label>
                                <select class="form-control" name="registration_card_exam_type[]" required>
                                    <option value="Youth Belt Exam">Youth Belt Exam</option>
                                    <option value="Adult Belt Exam">Adult Belt Exam</option>
                                    <option value="Makeup Youth Belt Exam">Makeup Youth Belt Exam</option>
                                    <option value="Makeup Adult Belt Exam">Makeup Adult Belt Exam</option>
                                </select>
                            </div>
                            <div class="col-md-4">
                                <label class="form-label">Order</label>
                                <input type="number" class="form-control" name="registration_card_order[]" 
                                       value="${newIndex + 1}" min="1" max="10" required>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <label class="form-label">Belt Colors (Visual Graphics)</label>
                                <select class="form-control" name="registration_card_belt_colors[${newIndex}][]" multiple size="5">
                                    <option value="white_orange">White w/Orange Stripe</option>
                                    <option value="white_black">White w/Black Stripe</option>
                                    <option value="orange">Orange</option>
                                    <option value="yellow">Yellow</option>
                                    <option value="green">Green</option>
                                    <option value="purple">Purple</option>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-4">
                                <label class="form-label">Button Text</label>
                                <input type="text" class="form-control" name="registration_card_button[]"
                                       value="Register Now" placeholder="e.g., Register Now" required>
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            <div class="col-md-12">
                                <div class="form-check">
                                    <input type="hidden" name="registration_card_invitation_only[${newIndex}]" value="0">
                                    <input type="checkbox" class="form-check-input" name="registration_card_invitation_only[${newIndex}]" 
                                           id="reg_invitation_only_${newIndex}" value="1">
                                    <label class="form-check-label" for="reg_invitation_only_${newIndex}">
                                        <strong style="color: #dc3545;">Invitation Only</strong> - Display "Invitation Only" badge on this registration card
                                    </label>
                                </div>
                            </div>
    ...
    // Initialize TinyMCE for lightbox content editors
    document.addEventListener('DOMContentLoaded', function() {
        tinymce.init({
            selector: '.tinymce-editor',
            plugins: 'link image lists table code preview fullscreen autolink charmap codesample emoticons hr pagebreak searchreplace visualblocks visualchars wordcount',
            toolbar: 'undo redo | blocks | bold italic underline strikethrough | forecolor backcolor | alignleft aligncenter alignright alignjustify | numlist bullist | link image | removeformat code fullscreen',
            height: 400,
            menubar: false,
            branding: false,
