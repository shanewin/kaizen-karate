...
if ($_POST) {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (isset($_POST['after_school_section'])) {
            ...
            // Handle PDF upload
            if (isset($_FILES['schedule_pdf_upload']) && !empty($_FILES['schedule_pdf_upload']['name'])) {
                if ($_FILES['schedule_pdf_upload']['error'] === UPLOAD_ERR_OK) {
                    $upload_result = handle_file_upload('schedule_pdf_upload', 'pdf', '../assets/images/aftersschool/');
                    if ($upload_result['success']) {
                        $schedule_pdf = $upload_result['path'];
                        $upload_successes[] = 'Schedule PDF uploaded successfully!';
                    } else {
                        $upload_errors[] = 'PDF upload failed: ' . $upload_result['error'];
                    }
                } else {
                    $upload_errors[] = 'PDF upload error (Code ' . $_FILES['schedule_pdf_upload']['error'] . ')';
                }
            }
            
            // Handle preview image upload
            if (isset($_FILES['schedule_image_upload']) && !empty($_FILES['schedule_image_upload']['name'])) {
                if ($_FILES['schedule_image_upload']['error'] === UPLOAD_ERR_OK) {
                    $upload_result = handle_file_upload('schedule_image_upload', 'image', '../assets/images/aftersschool/');
                    if ($upload_result['success']) {
                        $schedule_image = $upload_result['path'];
                        $upload_successes[] = 'Preview image uploaded successfully!';
                    } else {
                        $upload_errors[] = 'Image upload failed: ' . $upload_result['error'];
                    }
                } else {
                    $upload_errors[] = 'Image upload error (Code ' . $_FILES['schedule_image_upload']['error'] . ')';
                }
            }
            ...
            if (save_json_data('site-content', $content, 'draft')) {
                if (empty($upload_errors)) {
                    $msg = 'Weekend & Evening section saved to draft successfully!';
                    if (!empty($upload_successes)) {
                        $msg .= '<br>' . implode('<br>', $upload_successes);
                    }
                    $message = success_message($msg);
                } else {
                    $msg = 'Settings saved, but some uploads failed:<br>' . implode('<br>', $upload_errors);
