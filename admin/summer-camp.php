...
if ($_POST) {
    if (!verify_csrf_token($_POST['csrf_token'] ?? '')) {
        ...
        if (isset($_POST['summer_camp_section'])) {
            error_log('=== SUMMER CAMP POST RECEIVED ===');
            ...
            if (save_json_data('site-content', $content)) {
                error_log('=== SUMMER CAMP SAVE DEBUG ===');
                error_log('basic_title: ' . ($_POST['basic_title'] ?? 'NOT SET'));
                error_log('basic_subtitle: ' . ($_POST['basic_subtitle'] ?? 'NOT SET'));
                error_log('JSON saved successfully');
                error_log('================================');
        ...
        <div id="accordion-container">
            ...
            <div class="accordion-item border rounded p-4 mb-4">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <div>
                        <button type="button" class="btn btn-outline-secondary btn-sm me-1" onclick="moveAccordionSection(this, 'up')" title="Move Up">
                            <i class="fas fa-arrow-up"></i>
                        </button>
                        <button type="button" class="btn btn-outline-secondary btn-sm me-1" onclick="moveAccordionSection(this, 'down')" title="Move Down">
            ...
            <div class="row">
                <div class="col-md-6">
                    ...
                    <ul>
                        ...
                        <li><strong>Accordion Content:</strong> Manage all expandable information sections with rich text editing</li>
                    </ul>
                </div>
...
function removeAccordionSection(button) {
    ...
    // Remove TinyMCE instance first to avoid leaks
...
function moveAccordionSection(button, direction) {
    const item = button.closest('.accordion-item');
    const container = document.getElementById('accordion-container');
    
    // Check if move is possible
    if (direction === 'up' && item.previousElementSibling) {
        // We need to move the TINYMCE editor explicitly if using iframe mode, 
        // dragging DOM nodes can break iframes. Safest way is to remove editors, move DOM, re-init.
        saveAndRemoveEditors();
...
function updateAccordionNumbers() {
    ...
    accordionItems.forEach((item, index) => {
        const header = item.querySelector('h5');
        // Keep the inner icon but update the text
        header.innerHTML = `<i class="fas fa-chevron-down me-2 text-primary"></i>Section ${index + 1}`;
    });
}

// Helpers for reordering with TinyMCE
function saveAndRemoveEditors() {
    // Save content to textareas and remove editors
    if (typeof tinymce !== 'undefined') {
        tinymce.triggerSave();
        tinymce.remove();
        // Manually remove our initialization marker so they can be re-initialized
        document.querySelectorAll('.tinymce-editor').forEach(el => {
            el.classList.remove('mce-initialized');
        });
    }
}
